...
with open('data/chengdu_data/preprocessed_train_trips_small_osmid_hierarchy.pkl', 'rb') as f:
    train_data = pickle.load(f)
    f.close()


with open('data/chengdu_data/preprocessed_validation_trips_small_osmid_hierarchy.pkl', 'rb') as f:
    val_data = pickle.load(f)
    f.close()

with open('data/chengdu_data/preprocessed_test_trips_small_osmid_hierarchy.pkl', 'rb') as f:
    test_data = pickle.load(f)
    f.close()

# 构建数据集
for i in range(len(train_data)):
    if len(train_data[i]) == 3 and len(train_data[i][1]) != 0 :
        train_data[i] = ([train_data[i][1][0], train_data[i][1][-1], train_data[i][2][0]], train_data[i][1])

# 处理验证数据
for i in range(len(val_data)):
    # 检查 val_data[i] 是否包含三个元素，并且 val_data[i][1] 不是空
    if len(val_data[i]) == 3 and len(val_data[i][1]) != 0:
        val_data[i] = ([val_data[i][1][0], val_data[i][1][-1], val_data[i][2][0]], val_data[i][1])

# 处理测试数据
for i in range(len(test_data)):
    # 检查 test_data[i] 是否包含三个元素，并且 test_data[i][1] 不是空
    if len(test_data[i]) == 3 and len(test_data[i][1]) != 0:
...
#%% 读取node_nbrs
with open('data/chengdu_data/node_nbrs_hierarchy.pkl', 'rb') as f:
...
for epoch in tqdm(range(num_epoches)):
    ...
    print('loss:', loss)
#存储模型数据
torch.save(model.state_dict(), 'param/model_hierarchy.pth')
