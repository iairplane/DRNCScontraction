...
def contraction_hierarchy(graph, shortcut_pair, contraction_ratio=0.3):
    ...
    nodes_to_contract = []
    contracted_nodes = set()  # 用于追踪已收缩的节点
    ...
    for _ in tqdm(range(num_to_contract), desc="收缩节点进度", unit="节点"):
        sorted_nodes = sorted(shortcut_count.items(), key=lambda x: x[1])
        min_node, min_count = sorted_nodes[0]

        nodes_to_contract.append(min_node)
        contracted_nodes.add(min_node)  # 将当前节点标记为已收缩

        predecessors = list(contracted_graph.predecessors(min_node))
        successors = list(contracted_graph.successors(min_node))
        ...
        contracted_graph.remove_node(min_node)

        updated_neighbors = predecessors + successors
        for neighbor in updated_neighbors:
            if neighbor in contracted_graph:
                shortcut_count[neighbor] = calculate_shortcuts(contracted_graph, neighbor)

        if any(shortcut_count[neighbor] < min_count for neighbor in updated_neighbors):
            shortcut_count = {node: calculate_shortcuts(contracted_graph, node) for node in contracted_graph.nodes()}

        shortcut_count = {node: count for node, count in shortcut_count.items() if node in contracted_graph}
...
G, contracted_nodes = contraction_hierarchy(G,shortcut_pair, contraction_ratio=0.1)
...
for name in name_list:
    with open(f'data/harbin_data/preprocessed_{name}_trips_small.pkl', 'rb') as f:
